...
var agentAuthPassword string

var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Re-authenticate with the engine",
	Long: `Re-authenticate this agent with the engine.
...
If no config exists, run 'banyan-agent init' first.

Example:
  sudo banyan-agent auth
  sudo banyan-agent auth --password "my-cluster-secret"`,
	RunE: runAgentAuth,
}

func init() {
	rootCmd.AddCommand(authCmd)
	authCmd.Flags().StringVar(&agentAuthPassword, "password", "", "Cluster password (non-interactive mode)")
}

func runAgentAuth(cmd *cobra.Command, args []string) error {
	fmt.Println(styleTitle.Render("Banyan Agent - Re-authenticate"))
	...
	var password string
	if agentAuthPassword != "" {
		password = agentAuthPassword
	} else {
		form := huh.NewForm(
			huh.NewGroup(
				huh.NewInput().
	...
	engineAddr := fmt.Sprintf("%s:%s", cfg.Agent.EngineHost, cfg.Agent.EnginePort)
	...
	token, tokenErr := client.ExchangeToken(ctx, cfg.Agent.NodeName, "agent")
	if tokenErr != nil {
...
// validateAgentConfig checks that the agent config has the required fields
// for re-authentication.
func validateAgentConfig(cfg *types.BanyanConfig) error {
	if cfg.Agent.EngineHost == "" || cfg.Agent.EnginePort == "" || cfg.Agent.NodeName == "" {
		return fmt.Errorf("no agent config found. Run 'banyan-agent init' first")
	}
	return nil
}
