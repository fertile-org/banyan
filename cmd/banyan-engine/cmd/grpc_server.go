...
func startEngineGRPC(ctx context.Context, store storage.StateStore, port, passwordHash, registryURL string) (*engineGRPCServer, error) {
	...
	srv := grpc.NewServer(
		grpc.UnaryInterceptor(banyanrpc.PasswordAuthInterceptor(passwordHash)),
		grpc.StreamInterceptor(banyanrpc.PasswordAuthStreamInterceptor(passwordHash)),
	)
