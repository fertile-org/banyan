...
<CardGrid>
  ...
  <Card title="Simple tooling" icon="setting">
    Three focused binaries: `banyan-engine` for the control plane, `banyan-agent` for workers, `banyan-cli` for deployments. Install only what each server needs.
  </Card>
  ...
  <Card title="Proven foundations" icon="approve-check">
    gRPC for communication, etcd for state storage, containerd for running containers. No experimental runtimes.
  </Card>
</CardGrid>
