...
<div class="not-content" style="margin: 2rem 0; text-align: center;">
  ...
  <p style="margin-top: 0.75rem; font-size: 0.9rem; color: var(--sl-color-gray-3);"><code>banyan-cli dashboard</code> — monitor your cluster from the terminal or <code>--web</code> for a browser dashboard</p>
</div>
...
The **CLI** sends your manifest to the **Engine**, which stores state in etcd, manages encrypted secrets, and schedules containers across **Agents** (with auto-scaling). Each Agent runs containerd and pulls images from the Engine's built-in registry. All communication is encrypted with WireGuard. For high availability, run [multiple engines](/guides/high-availability/) — they coordinate automatically.
